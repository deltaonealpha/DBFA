...
def Login():
    ...
    with open(creds, 'r') as f:
        ...
        if ((CAPSLOCK) & 0xffff) != 0:
            ...
            layout = [  [sgx.Text('Login for access')],
                        [sgx.Checkbox('Employee', key='baseperms'), sg.Checkbox('Administrator', key='rootperms')],
            ...
            layout = [  [sgx.Text('Login for access')],
                        [sgx.Text('User profile: '), sgx.Checkbox('Employee', key='baseperms'), sg.Checkbox('Administrator', key='rootperms')],
                        [sgx.Text('Username: '), sgx.InputText()],
                        [sgx.Text('Password: '), sgx.InputText(password_char='*')],
                        [sgx.Button('Login'), sgx.Button('Cancel')]]
        # Create the Window

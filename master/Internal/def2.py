...
#Rough algorithm design:
        # Get average week/ past-month hours
        # Fetch empty schedule design
        # (Treat resignations as leave for scheduling)
        # Schedule design to have few PERMANENT spots (posts) and interchangable sub-spots (for the same posts)
        # Arrange in accordance to net hours to touch 56 hours per week and give overtime if one's on leave
        # If a leave is applied for, adjust by giving half overtime to the earlier slot's worker and half to the next slot's worker
# Send the next day's shift schedule daily post verifying status with the db.
# Do salary generation/ payment check and display in main menu/ emp man menu ??????
...
This is dynamically generated with alternating shifts and takes care of employees on leave.
'''
